...
\begin{description}
\item[Reprezentace času] Jednotlivé stavy geodat jsou odkazovány k  časovým okamžikům dvěma způsoby a to relativně  (\textit{dny,roky, minuty})  či absolutně (\textit{1990-30-06 1:35:59}). Možností relativní reprezentace je vyjádření času záporně (\textit{-rok}). Další reprezentace je vyjádřena pomocí klasického dělení časů (\textit{\textbf{před}- minulý, \textbf{teď}- současný, \textbf{potom}- budoucí}).
\item[Diskrétně časový model] je takový model, kde se proměnné, tedy geodata  mění v skokově. Geodata jednotlivých okamžiků na sebe navazují podle časového rozlišení dostupných geodat. Vhodným příkladem je sčítání lidu v daném intervalu
...
\subsubsection{Kompozitně-časový model}
...
\subsubsection{Time-stamping model} 
...
\subsubsection{Objektově orientovaný model}
