...
enviromentálního modelování. Jestliže se poohlédneme do první poloviny
...
například odtokových procesů. Oproti rozvoji fyzikálně numerických modelů
...
odtokově senzitivních oblastech jako jsou městské aglomerace. Pokrokové 
operativní metody určování srážek metodou MV spojů představují perspektivní 
využití především v hydrologii urbanizovaných povodí, kde (díky hustotě MV sítí) 
mohou sloužit jako podklad vodohospodářských dispečinků operativního řízení při 
řešení kalamitních povodňových situací v síti dešťové kanalizace. 
...
společnostmi T-Mobile a Veolia ČR a nově s Ericsson Research
(Sweden).
...
modulech jsou také globální přepínače, které jsou pro všechn)y moduly
...
\item[Exekutor] pracuje pouze s  \acs{DML}. Obdrží od výše zmíněného
  ...
  procesů zpracovávat od horního uzlu.
\end{description}
...
vytvoření vlastních funkcí, či využití funkci z standardní knihovny
PostgreSQL.
...
Jedno z běžných použití je implementace pohledů, které jsou logicky odkazují na vybrané sloupce v
...
jevů je nedílnou součástí časová složka. Z historického pohledu na rozvoj 
systémů GIS je zřejmá jejich původní orientace na řešení problémů navazujících 
na analýzy prostorových dat. V prvních fázích rozvoje GIS nemohla ani být 
...
Následující text představuje základní terminologii časoprostorové
...
\begin{figure}[h!]
 ...
 zaznamenávány do databáze v rámci malých intervalů nahodile, což je způsobeno přenosovými rychlostmi dat a jejich  zápisem do databáze. Nehledě na skutečnost, že sběr dat probíhá  
 nahodile, systém garantuje pro jejich ukládání do databáze interval sběru 
 cca 15 (s).
...
pro jiná využití. Výstupní formát modulu podporující \textit{Temopral GRASS
