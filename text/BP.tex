...
{\tt isettings}                              & string                  &                              \\
%%% ML: integerstring ?
{\tt pmethod}                                & integerstring           & permeter, count              \\
{\tt step}                                   & integer                 &                              \\
...
možné editovat barevné škály modulem \textit{r.color} pro rastrová data
...
možným výsledkům značně ovlivněny. Možným řešením problému rozložení
bodů je volba výpočetního regionu jen pro jeden referenční vysílač, to
by ale reálnou situaci také nedemonstruje.
...
Metoda IDW v GRASS nedisponuje některými nastaveními, které by byla vhodná
...
formě návodu, který bude sloužit jako úvod pro experimentálního využití
...
\emph{g.gui.timeline}, který na časové ose zobrazuje zvolené
...
%  \texttt{input} a \texttt{type} žádné další parametry.
 ...
 čase může být využito například při změně času nebo při praci s
...
Vzhledem k tomu, že se jedná o vývoj SW prostředku jako jednoho z
modulů komplexního programu řešeného v rámci mezinárodního projektu,
nebyla v rámci této práce zdůrazněna aplikační hodnota vyvinutého
programu. Závěrem lze však konstatovat, že dílčí modul vyvíjeného SW
dostatečně prokázal v rámci případové studie svou funkčnost a splnil
předpoklad vytvoření datového rozhraní časoprostorové data\-báze
meteorologických a hydrologických podkladů.
