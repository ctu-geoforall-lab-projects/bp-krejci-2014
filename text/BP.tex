...
modelů reálného světa. Pro modelování některých reálných
...
\item[Diskrétně časový model] je takový model, kde se proměnné, tedy
  ...
  rozlišení (granularity). Vhodným příkladem je sčítání lidu v daném časovém
  intervalu.
...
specifické nároky vědních oborů vytvořily poměrně velikou škálu
...
vhodné struktuře pro implementaci do GIS. Jak již bylo zmíněno, model
...
separovaných vrstev \ref{fig:snapshot}.  Model charakteristikou svého navržení odpovídá
diskrétním systémům. Jednotlivé proměnné - geodata se mění v
průběhu času skokově. Z toho mimo jiné pro GIS plyne, že
...
procházení a porovnávání buňky po buňce, což vede k vysoké operační
náročnosti.

\paragraph*{Time Composit}
\textit{\ac{STC}} byl navrhnut  v roce
1988, Lagran \cite{lagran}. Model \acs{STC} pracuje s vektorovou interpretací
 prostorových dat. Princip tohoto modelu spočívá v projekci linií do roviny, čímž se v
...
\textit{Event-Oriented Model}, je svým principem blízký  předchozího
modelu. Model \textit{Time-stamping }nedokáže identifikovat jednotlivé změny v
...
jednotlivých změn. Object-Relationship model je tedy  specifický dle zaměření
dané problematiky. Specifikem modelu je  náročnost v definování
jednotlivých vztahů mezi objekty.

\paragraph*{Objektově orientovaný model}
\textit{Object-Oriented model}  je principiálně podobný charakteristice objektově 
orientovaných programovacích jazyků. Tato koncepce umožňuje vytvářet
...
2D/3D mapami. Jednotlivé objekty s časovou charakteristikou jsou
ukládány do časových datasetů. Ty pak přiřazují prostorové
...
jako ve zmíněné terminologii viz \ref{subsec:terminologie} Terminologie.

\paragraph*{Granularita} - časová jemnost nebo časové 
...
obsahují mezery v čase (gaps) ve kterých nejsou dostupná data.
...
V části \textit{GRASS GIS modul} \ref{sec:grassgismodule} jsou popsány
...
založena na předpokladu, že bude sloužit k experimentálním pokusům v
rozhraní GRASS GIS. Jednotlivé kroky výpočtů jsou od sebe odděleny, tak
aby se při změně parametrů  části programu znovu nemusely provádět
veškeré výpočty. Tyto kroky dělí se do výše zmíněných tří kategorií.
