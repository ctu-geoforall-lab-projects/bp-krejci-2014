...
Pochopení základních  principů, na  kterých jsou interpolace založené, může přispět k dosažení kvalitních výsledků v analytické části. Důležité je především znát volitelné parametry metod, které je potřebné v nástrojích GIS nastavit. Následující text je zaměřen na interpolační metody, kterými disponuje systém GRASS GIS pro vektorová bodová data. 
...
kde $T(x)$ je trend funkce, $lambda_{j}$ jsou neznámé koeficienty daných bodů, které se řeší soustavou lineárních rovnic. Funkce $R(r,r_{j})$ je radiální funkcí explicitně závislé na koeficientu vyhlazení.
