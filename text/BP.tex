...
%%% ML: odkaz na to co je TeleMAS.
%%% MK: Telemas je vnitrni nazev projektu. nedam to tam
...
%%% MK: Myslis ze by tu tato tabulka mela byt? Moc sem nesedi hlavne z pohledu formatovani...
\begin{comment}
...
Úprava databáze se provádí v prostředí navrženého GRASS modulu a je
charakterizována následujícím postupem:
\begin{enumerate}
%%% ML: odkaz na kapitolu o PostGISu
%%% MK: opraveno
\item \textbf{Geometrie} - extenze PostGIS\ref{subsec:postgis},
...
{\tt link}. Souřad\-nicový systém je WGS-84 (EPSG 4326).

Vytvoření geometrického atributu ({\tt geom}) datového typu POINT pro
vysílače.
...
%%% ML: zminku o GRASS Py lib jsem odtranil, nemela souvislost k UI
%%% MK: ok
...
představující jednotlivá \uv{časová okna}, vhodná pro následné
analýzy v GIS.
...
%%% ML: veta nize nedava prislis smysl, neco tam chybi...
%%% MK: opraveno
...
%%% ML: zkus vetu prepsat, na prvni pohled nedava prilis smysl...
%%% MK: smazal jsem to, nemuselo to tam byt
...
%%% MK:  Jasne
zadavatele\footnote{Ing. Martin Fencl, řešitel projektu: GA ČR 14-22978S \uv{Predikce srážkového odtoku v
  urbanizovaných povodích na základě deštěm generovaného útlumu
...
Prvotní pochopení nároků na daný GIS modul nasvědčovali výhodě koncipovat 
psaní kódu procedurálně. V závěru psaní aplikace bylo zjištěno, že objektově 
orientovaný návrh by byl vhodnější a to především pro zvyšující se počet funkci
...
metod jejího určování.  Moduly které vznikly v rámci této práce byly přidány do 
 \textit{GRASS Add-ons repository} 
a jsou současně volně dostupny všem uživatelům GRASSu. 
...
Ke stažení zdrojových kódů je se využívá systému \textit{Subversion}. Instalaci lze spustit daným příkazem v terminálu.
\begin{figure}[h!]
\begin{scriptsize}
\lstset{extendedchars=false,
escapeinside=''}
...
\end{figure}
\vskip-2ex
Dalším krokem je stažení zdrojového kódu:
\begin{figure}[h!]
\begin{scriptsize}
\lstset{extendedchars=false,
escapeinside=''}
...
GRASS GIS potřebuje má závislosti na externích knihovnách. Ty je možné snadno nainstalovat pomocí balíčků:
\begin{figure}[h!]
\begin{scriptsize}
\lstset{extendedchars=false,
escapeinside=''}
...
Nejdříve se přesuneme do složky grass71\_release.
\begin{figure}[h!]
\begin{scriptsize}
\lstset{extendedchars=false,
escapeinside=''}
...
\end{figure}
\vskip-2ex
Dalším krokem je konfigurace spuštěním příkazu:
\begin{figure}[h!]
\begin{scriptsize}
\lstset{extendedchars=false,
escapeinside=''}
...
Instalaci modulů z repozitáře GRASS Add-ons, kde jsou uloženy i moduly vzniklé v rámci této práce, lze stáhnout přímo z prostředí systému GRASS GIS. Ke stahování modulů z externích Subversion repozitářů slouží modul \textit{g.extension}.
...
escapeinside=''}
\begin{lstlisting}[style=mybash]
g.extension extension=r.mwprecip operation=add  
g.extension extension=v.link.precip operation=add                                                       
\end{lstlisting}
\end{scriptsize} 
\end{figure} 
\vskip-2ex

Modul lze pak spustit ze standardního vstupu GRASS např:
\begin{figure}[h!]
\begin{scriptsize}
\lstset{extendedchars=false,
escapeinside=''}
\begin{lstlisting}[style=mybash]
r.mwprecip.py database=letnany interval=day                                             
\end{lstlisting}
\end{scriptsize} 
\end{figure} 
...
Data jsou uložena v db. PostgreSQL, jejíž název je \textit{letnany}. Cílem zadání je vytvořit \textbf{rastry plošných interpolací} v časovém kroku 1 minuta. Tyto rastry nás zajímají pouze z \textbf{časového intervalu} od 2013-09-08 23:59:00 do 2013-09-09 23:59:00. Ze stejného období pro následující časoprostorové analýzy potřebujeme vytvořit časová okna z časových řad srážkoměrů. Pro výpočet baseline známe  časové intervaly nulových srážek (suchá období). Z těchto suchých období chceme určit baseline z horního 96 \% \textbf{kvantilu} daných hodnot. Máme informaci o \textbf{rozbitých spojích} linkid= 63, 58 a 71. Jejich hodnoty pro odhady plošných srážek chceme ignorovat. K plošným interpolacím využijeme metody IDW s exponentem 1.6 a počtem bodů zasahujícím do výpočtu 35. Vizualizaci rastru nastavíme vlastní \textbf{tabulkou barev}. K nahrazení linií body využijeme nahrazení linie \textbf{dvěma body rovnoměrně}. Celý výpočet uložíme do \textbf{pracovního schema} \textit{ukazka}.
...
\begin{figure}[h!]
    ...
    \caption[TW]{Snímek okna modulu znázorňující konfiguraci určení baseline  \centering  }
 \end{figure}
