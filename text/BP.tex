...
\item \textbf{interpolace} - vytvoření plošných inter{subsec:postgresql}polací z časových oken
\end{enumerate}
...
Uživatelské rozhraní se primárně dělí na již zmíněné čtyři části a následující text se je podlé tohoto dělení strukturován.

\subsubsection*{Data}
V následujícím textu je popsána příprava databáze a filozofie správy dat. 

\paragraph*{Příprava databáze}
 se provádí automaticky v prostředí navrženého GRASS modulu a 
...
Druhý způsob  je umožněn pomocí volby suchého období a z něj spočtení hodnoty baseliine na základe statistických funkcí. Schéma volby baseline je znázorněno na obrázku.  \ref{fig:schemabase}.
...
Volba suchých období se definuje do textového souboru který je vstupní hodnotou do parametru \texttt{baseltime}.
Pro příklad: 
...
\begin{itemize}
\item průměr (avg) - určení hodnot baseline prostým průměrováním hodnot pro jednotlivé spoje
\item modus (mode) - určení pomocí funkce modus. Z podstaty této funkce je nutno nastavit zaokrouhlení (parametr \texttt{roundm}) dat, které je provedeno před samotným výpočtem funkce modus.
\item quantile (kvanitl) - určení hodnot uživatelsky nastaveným kvantilem. Hodnota kvantilu se nastavuje parametrem \texttt{quantile}.  
\end{itemize}
...
Časová okna jsou druhou samostatnou GUI sekcí s názvem
...
\item generování časových oken ze zvoleného  intervalu
...
Časový úsek, ze kterého jsou časová okna vytvářena je vymezen krajními hodnotami: \textit{fromtime}
...
Plošné interpolace srážek jsou  jedním z hlavních výstupů modulu
\textit{r.mwprecip}. GUI sekce \textit{Interpolation} je charakteristická
dvěma částmi: 
\begin{itemize}
\item interpolace bodů podél spojů
\item plošné interpolace srážek
\end{itemize}

\paragraph*{Volba regionu}
