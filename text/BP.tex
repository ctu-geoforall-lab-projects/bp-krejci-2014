...
Jednotlivé podrobnosti dílčích funkcí spadajících do těchto kategorií
jsou podrobně rozepsány v podkapitole (\ref{subsub:pouzitimodulurmw}, Použití modulu).
...
\paragraph{Datový vstup} do modulu představuje konektivita s externí 
...
\paragraph*{Datový výstup} Jedním ze základních výstupů jsou vypočtené srážkové intenzity uložené
...
představující jednotlivá \uv{časová okna} (atributové tabulky), vhodná pro následné
analýzy v GIS.

\textbf{Rastrová data} reprezentují výsledek zvolené plošné
interpolace vypočtených srážek. \textbf{Vektorová data} jsou vytvořena na základě polohových údajů o
...
\textit{v.link.precip} připojit jednotlivá \uv{časová okna} (atributové tabulky) z databáze.
 
\subsubsection*{Použití modulu}
\label{subsub:pouzitimodulurmw}
V následujícím textu jsou představeny obecně jednotlivé specifika vlastních funkcí modulu
\textit{r.mwprecip} a jejich možná konfigurace. Konkrétní ukázka nastavení modulu
...
\textit{v.link.precip} připojována jednotlivá \uv{časová okna} z databáze.
...
zmíněným principem připojit příslušných \uv{časová okna} ze zvoleného okamžiku.
\end{itemize} 
