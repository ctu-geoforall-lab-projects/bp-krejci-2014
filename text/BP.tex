...
PostGIS\footnote{\url{http://postgis.net/}} je open-source rozšířením \texttt{EXTENSION} databáze PostgeSQL o podporu geografických objektů. Jde tedy o velice vhodný   nástroj pro využití v GIS. PostGIS implementuje specifikaci \textit{Simple Features for SQL}\footnote{\url{http://portal.opengeospatial.org/files/?artifact_id=829} } konsorcia \acs{OGC}, která v současné době plní funkci standardizační organizace pro geoprosotrová data. PostGIS je vektorovou databázovou nadstavbou, vedle které je i extenze PostGIS Raster. 
...
\paragraph*{Geometrie} \texttt{geometry} je datovým typem, který představuje geometrickou složku objektu.  Charakteristika tohoto datového typu je dána standardem \textit{Simple Features} ve formátu \acs{WTK}.
...
Pro definování \textbf{typu} objektu jsou k dispozici funkce \texttt{ST\_Make* }, kde \texttt{*} jsou níže uvedené typy Simple Fetures. 
...
\paragraph*{Funkce} v této nadstavbě jsou velice rozsáhlého. Veškeré funkce mají prefix \texttt{ST\_} (\textit{spatial type}).  Svým účelem analogicky čerpají z nástrojů GIS. Funkce jsou podle svého účelu roztříděny do kategorii. Níže jsou demonstrovány nejzákladnější kategorie a funkce.
\begin{description}
